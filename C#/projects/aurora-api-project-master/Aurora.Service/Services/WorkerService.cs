...
namespace Aurora.Service.Services
{
    public class WorkerService : IServiceWorker
    {
        ...
        public WorkerModel Update(int id, UpdateWorkerModel userModel)
        {
            if (id != userModel.Id)
            {
                _notificationContext.AddNotifications(new Contract().AreNotEquals(id, userModel.Id, nameof(id), "Usuário não identificado."));
            ...
            _repositoryUser.Save(user);                     // tanto usuário novo como já existente, utiliza-se save.
            return user.ConvertToUser();
        }
    }
}
